...
import Basil.Client;
import Basil.module.movement.Tower;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
    	//Basil
    	for (Basil.module.Module m: Client.moduleManager.getModules()) {
    		m.playerHitBy(source);
    	}
            ...
            else
            {
                ...
                if (flag)
                {
                    ...
                    if (entity != null)
                    {
                        ...
                        //Basil
                        if (Client.moduleManager.getModuleByName("Velocity").isToggled() && this instanceof EntityPlayer) {
                        	d1 *= Client.sm.getSettingsByMod(Client.moduleManager.getModuleByName("Velocity")).get(0).getValDouble();
                        	d0 *= Client.sm.getSettingsByMod(Client.moduleManager.getModuleByName("Velocity")).get(0).getValDouble();
                        }
    ...
    public void knockBack(Entity entityIn, float p_70653_2_, double p_70653_3_, double p_70653_5_)
    {
        if (this.rand.nextDouble() >= this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).getAttributeValue())
        {
            ...
            //Basil
            if (Client.moduleManager.getModuleByName("Velocity").isToggled() && this instanceof EntityPlayer) {
            	f1 *= Client.sm.getSettingsByMod(Client.moduleManager.getModuleByName("Velocity")).get(1).getValDouble();
            }
    ...
    {
    	//Basil
    	if (Client.moduleManager.getModuleByName("Tower").isToggled()) {
    		return 0.369F;
    	}
        return 0.42F;
    }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isJumping)
        {
            ...
            else if (this.onGround && this.jumpTicks == 0)
            {
                this.jump();
                //Basil
            	if (!Client.moduleManager.getModuleByName("Tower").isToggled() && !Client.moduleManager.getModuleByName("FastJump").isToggled()) {
                	this.jumpTicks = 10;
            	}
            }
        }
