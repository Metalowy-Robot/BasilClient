...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    {
    	//Basil
    	if (((FreeCam)Client.moduleManager.getModuleByName("FreeCam")).capture) {
    		return;
    	}
    	if (p_147297_1_ instanceof C03PacketPlayer && ((Blink)Client.moduleManager.getModuleByName("Blink")).capture) {
    		((Blink)Client.moduleManager.getModuleByName("Blink")).packets.add((C03PacketPlayer) p_147297_1_);
    		return;
    	}
    	if (((FakeLag)Client.moduleManager.getModuleByName("FakeLag")).capture) {
    		long lagMin = (long) Basil.Client.sm.getSettingsByMod(Client.moduleManager.getModuleByName("FakeLag")).get(0).getValDouble();
    		long lagMax = (long) Basil.Client.sm.getSettingsByMod(Client.moduleManager.getModuleByName("FakeLag")).get(1).getValDouble();
    		long lag = (long) (lagMin + Math.random() * (lagMax - lagMin));
    		((FakeLag)Client.moduleManager.getModuleByName("FakeLag")).packets.add(p_147297_1_);
    		((FakeLag)Client.moduleManager.getModuleByName("FakeLag")).packetsTime.add(System.currentTimeMillis() + lag);
    		return;
    	}
